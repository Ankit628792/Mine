...
export default class OTPInputView extends Component {
  ...
  checkPinCodeFromClipBoard = () => {
    Clipboard.getString()
      .then(code => {
        if (
          this._hasCheckedCode &&
          code.length === this.props.pinCount &&
          this._code !== code
        ) {
          this.setState(
            {
  ...
  _renderOneInputField = index => {
    ...
    const {defaultTextFieldStyle} = StyleSheet.create({
    ...
    return (
      // <TouchableHighlight pointerEvents="none" key={index + "view"}>
      <TextInput
