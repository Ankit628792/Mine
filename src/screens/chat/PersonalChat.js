import { View, Text, Image, Dimensions, TouchableOpacity, TextInput, FlatList, TouchableHighlight, ScrollView, Keyboard, ActivityIndicator } from 'react-native'
...
const ChatItem = React.memo(({ item, mine, last, next }) => {
    ...
    return (
        item?.last
            ?
            <View style={tw`w-full h-5 mt-8`}>
            </View>
            :
            <TouchableHighlight activeOpacity={1} underlayColor='none' style={tw`${mine ? 'flex-row-reverse' : 'flex-row'} items-start ${last ? 'mt-0' : 'mt-8'}`}>
                <>
                    <View style={[tw`w-11 h-11 rounded-full overflow-hidden shadow-md ${mine ? 'ml-2 bg-gray-100' : 'mr-2 bg-gray-800'} ${last ? 'opacity-0' : ''}`]}>
                        <Image source={{ uri: image }} resizeMode='cover' style={tw`w-full h-full`} />
                    </View>
...
const PersonalChat = ({ route }) => {
    ...
    return (
        <>
            <LinearGradient style={tw`flex-1 p-5 relative`} colors={['#00d2ff', '#3a7bd5']}>
                        ...
                        messages?.length
                            ...
                            <>
                                <ScrollView showsVerticalScrollIndicator={false} ref={flatListRef}>
                                    {messages?.map((item, index) => <ChatItem key={index} item={{ ...item, image: item?.sender == user?._id ? user?.image : receiver?.image }} mine={item?.sender == user?._id} last={messages[index - 1]?.sender == messages[index]?.sender} next={messages[index + 1]?.sender != messages[index]?.sender} />)}
                                    <View style={tw`w-full h-5 mt-8`}>
                                    </View>
                                </ScrollView>
                                {/* <FlatList
                ...
                <View style={tw`bg-white rounded-full flex-row items-center justify-between px-2 mt-auto`}>
                    <TextInput returnKeyType={text ? 'send' : 'none'} onSubmitEditing={({ nativeEvent: { text } }) => handleSendMessage(text)} value={text} onChangeText={setText} style={tw`text-base text-gray-600 w-full ml-1 h-auto max-w-[${Dimensions.get('window').width - 110}px]`} placeholder='Type here...' placeholderTextColor="#999" />
                    <TouchableOpacity disabled={!text?.length} style={tw`w-11 h-11 p-2 ${text?.length ? 'opacity-100' : 'opacity-0'}`} onPress={() => handleSendMessage(text)}>
                        <Svg viewBox="0 0 24 24" fill='currentColor' style={tw`text-sky-500`}>
