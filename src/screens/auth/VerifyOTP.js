...
const VerifyOTP = ({ route }) => {
    ...
    return (
        <TouchableWithoutFeedback onPress={() => Keyboard.dismiss()}>
            <LinearGradient style={tw`flex-1 flex-col justify-between relative p-5 pb-10`} colors={gradient.orange} >
                <BackButton />
                <View></View>
                <View style={[tw`p-5 absolute bottom-0 left-0 right-0 rounded-t-3xl`, { backgroundColor: colors.white }]}>
                    ...
                    <View style={tw`flex-col items-center justify-center mb-10`}>
                        ...
                        <View style={tw`relative w-4/5 z-10 mt-6`}>
                            <OTPInputView
                                ...
                                pinCount={6}
                                code={otp}
                                codeInputFieldStyle={{ color: otp?.length == 6 ? colors.orange : colors.black }}
                    ...
                    <View style={tw`px-5`}>
                        <PrimaryButton text={'Submit'} disabled={isLoading || otp.length !== 6} isLoading={isLoading} onPress={() => checkOtp()} />
                    </View>
                </View>
            </LinearGradient>
