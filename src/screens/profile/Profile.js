...
const TabBarCustom = (props) => {
  const { navigationState, position, tabBarStyle, labelStyle } = props;
  return (
    <TouchableOpacity style={[tw`flex-row w-full px-10 gap-10`, tabBarStyle]}>
      {navigationState.routes.map((route, index) => {
        const isFocused = index === navigationState.index;
        const textColor = isFocused ? colors.orange : colors.darkGray;
        return (
          <Text
            key={route.key}
            onPress={() => {
            ...
            style={[tw`text-xl text-center flex-grow border-b py-2`, { color: textColor, borderColor: textColor }, labelStyle]}
          >
            {route.title}
          </Text>
        );
