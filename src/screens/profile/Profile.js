...
const Profile = () => {
  ...
  const [userData, setUserData] = useState({
    fname: 'Test',
    lname: 'User',
    userImg: 'https://w0.peakpx.com/wallpaper/470/485/HD-wallpaper-the-batman-robert-pattinson-the-batman-batman-superheroes-movies-2021-movies-robert-pattinson.jpg',
  ...
  return (
    <LinearGradient colors={gradient.purple} style={tw`flex-1`}>
      <View style={tw`p-5 pb-2`}>
        <Text style={[tw`text-2xl font-semibold mb-3 ml-3 text-white text-center`]}>Profile</Text>
      </View>
      ...
      <View style={[tw`p-5 flex-1`, { borderRadius: 40, backgroundColor: colors.white }]}>
        <ScrollView showsVerticalScrollIndicator={false} style={tw`p-2`}>
          <View
            ...
            <Text style={[tw`text-2xl font-medium mt-2 text-center`, { color: colors.black }]}>Batman</Text>
            <Text style={[tw`text-lg mt-1 text-center`, { color: colors.gray }]}>+91 8985464414</Text>

            <TouchableOpacity onPress={() => navigator.navigate('ViewProfile', { user: userData })} style={tw`bg-white flex-row w-full items-center justify-between py-3 px-5 my-2 mt-10 rounded-md shadow-lg shadow-gray-300`}>
